...
import trader # Import nécessaire pour utiliser trader.LEVERAGE
...
def setup_database():
    ...
    cursor = conn.cursor()# Fichier: database.py
import sqlite3
import time
from typing import List, Dict, Any, Optional
...
def get_last_closed_trade() -> Optional[Dict[str, Any]]:
    """Récupère le dernier trade clôturé."""
    conn = get_db_connection()
    cursor = conn.cursor()
...
def get_all_closed_trades() -> List[Dict[str, Any]]:
    """Récupère tous les trades clôturés."""
    conn = get_db_connection()
    cursor = conn.cursor()
...
def update_trade_sl(trade_id: int, new_sl_price: float):
    """Met à jour le prix du SL pour un trade donné."""
    conn = get_db_connection()
    cursor = conn.cursor()
