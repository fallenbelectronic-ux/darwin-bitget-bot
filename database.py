...
def create_trade(symbol: str, side: str, regime: str, status: str, entry_price: float, sl_price: float, tp_price: float, quantity: float, risk_percent: float, open_timestamp: int, bb20_mid_at_entry: Optional[float]):
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    sql = "INSERT INTO trades (symbol, side, regime, status, entry_price, sl_price, tp_price, quantity, risk_percent, open_timestamp, bb20_mid_at_entry) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
...
def close_trade(trade_id: int, status: str, pnl: float):
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE trades SET status = ?, pnl = ?, close_timestamp = ? WHERE id = ?", (status, pnl, int(time.time()), trade_id))
    conn.commit()
    conn.close()
...
def update_trade_tp(trade_id: int, new_tp: float):
    """Met à jour le Take Profit d'un trade ouvert."""
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE trades SET tp_price = ? WHERE id = ?", (new_tp, trade_id))
    conn.commit()
    conn.close()
