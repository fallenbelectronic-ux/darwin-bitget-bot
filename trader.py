...
import notifier
import charting
import utils  # ✅ on passe par utils.fetch_ohlcv_df pour éviter l'ImportError
...
def detect_signal(symbol: str, df: pd.DataFrame) -> Optional[Dict[str, Any]]:
    ...
    for i in range(2, 4):
        ...
        buy_ct = (contact_candle['low'] <= contact_candle['bb20_lo'] and
                  contact_candle['low'] <= contact_candle['bb80_lo'])
        ...
        sell_ct = (contact_candle['high'] >= contact_candle['bb20_up'] and
                   contact_candle['high'] >= contact_candle['bb80_up'])
...
def close_position_programmatically(ex: ccxt.Exchange, trade: Dict, reason: str, exit_price: Optional[float] = None):
    ...
    database.close_trade(trade['id'], status=f'CLOSED_{reason.upper()}', exit_price=float(exit_price))
    notifier.send_programmatic_closure_notification(trade['symbol'], trade['side'], reason, float(exit_price))

def manage_open_positions(ex: ccxt.Exchange):
    """Gère les positions ouvertes, incluant la clôture sur signal inverse."""
    is_paper_mode = database.get_setting('PAPER_TRADING_MODE', 'true') == 'true'

    for pos in database.get_open_positions():
        df = utils.fetch_ohlcv_df(ex, pos['symbol'], TIMEFRAME)  # ✅
        if df is None or len(df) < 83:
            continue
        ...
        if signal:
            ...
            if (is_long_position and is_short_signal) or ((not is_long_position) and (signal['side'] == 'buy')):
        ...
        if pos.get('management_strategy') == 'SPLIT' and pos.get('breakeven_status') == 'PENDING':
            try:
            ...
            new_dynamic_tp = (
                last_indicators['bb80_mid'] if pos['regime'] == 'Tendance'
                else last_indicators['bb20_up'] if is_long
                else last_indicators['bb20_lo']
            )
            if new_dynamic_tp and (abs(new_dynamic_tp - pos['tp_price']) / pos['tp_price']) * 100 >= TP_UPDATE_THRESHOLD_PERCENT:
                try:
                    ...
                    ex.private_post_mix_position_modify_position(params)
                    database.update_trade_tp(pos['id'], float(new_dynamic_tp))
                except Exception as e:
...
def execute_trade(ex: ccxt.Exchange, symbol: str, signal: Dict[str, Any], df: pd.DataFrame, entry_price: float):
    ...
    if len(database.get_open_positions()) >= max_pos:
        rejection_reason = f"Rejeté: Max positions ({max_pos}) atteint."
    elif database.is_position_open(symbol):
        rejection_reason = "Rejeté: Position déjà ouverte (DB)."
    else:
        try:
    ...
    # ✅ aligne l'API database.create_trade (signature corrigée)
    entry_atr = float(df.iloc[-1].get('atr', 0.0))
    entry_rsi = float(df.iloc[-1].get('rsi', 0.0)) if 'rsi' in df.columns else 0.0
    database.create_trade(
        symbol, signal['side'], signal['regime'],
        float(final_entry_price), float(signal['sl']), float(signal['tp']),
        float(quantity), float(RISK_PER_TRADE_PERCENT),
    ...
    notifier.tg_send_with_photo(
        photo_buffer=chart_image,
        caption=trade_message,
        chat_id=notifier.TG_ALERTS_CHAT_ID or notifier.TG_CHAT_ID
    )
...
def close_position_manually(ex: ccxt.Exchange, trade_id: int):
    ...
    exit_price = ex.fetch_ticker(trade['symbol'])['last']
    close_position_programmatically(ex, trade, "MANUAL", float(exit_price))
