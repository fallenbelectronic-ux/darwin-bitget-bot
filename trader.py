...
def is_valid_reaction_candle(candle: pd.Series, side: str) -> bool:
    ...
    if side == 'buy':
        if candle['close'] <= candle['open']:
            print(f"  -> INFO: Réaction d'achat ignorée (bougie de réaction rouge).")
            return False
    ...
    if side == 'sell':
        if candle['close'] >= candle['open']:
            print(f"  -> INFO: Réaction de vente ignorée (bougie de réaction verte).")
            return False
...
def manage_open_positions(ex: ccxt.Exchange):
    """Gère les positions ouvertes selon leur stratégie de gestion."""
    ...
    for pos in database.get_open_positions():
        ...
        # GESTION TRAILING STOP ATR (TENDANCE)
        if pos['regime'] == 'Tendance':
            new_trailing_sl = 0
            if is_long:
        ...
        # GESTION TP DYNAMIQUE (TENDANCE OU 2EME PARTIE SPLIT)
        else:
...
def execute_trade(ex: ccxt.Exchange, symbol: str, signal: Dict[str, Any], df: pd.DataFrame, entry_price: float) -> Tuple[bool, str]:
    ...
    if current_strategy_mode == 'SPLIT' and signal['regime'] == 'Contre-tendance': management_strategy = "SPLIT"
        
    database.create_trade(symbol, signal['side'], signal['regime'], final_entry_price, signal['sl'], signal['tp'], quantity, RISK_PER_TRADE_PERCENT, management_strategy, signal.get('entry_atr', 0), signal.get('entry_rsi', 0))
...
def close_position_manually(ex: ccxt.Exchange, trade_id: int):
    ...
    try:
        ...
        database.close_trade(trade_id, status='CLOSED_MANUAL', exit_price=exit_price)
