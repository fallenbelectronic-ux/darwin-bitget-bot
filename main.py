...
import traceback
import threading
from ta.volatility import BollingerBands  # (garde pour compat éventuelle)
from typing import List, Dict, Any, Optional
from datetime import datetime, timezone
import pytz
from tabulate import tabulate  # ✅ utilisé dans /history
...
API_KEY, API_SECRET, PASSPHRASSE = (
    os.getenv("BITGET_API_KEY", ""),
    os.getenv("BITGET_API_SECRET", ""),
    os.getenv("BITGET_API_PASSWORD", "") or os.getenv("BITGET_PASSPHRASSE", ""),
)
TIMEFRAME, UNIVERSE_SIZE, MIN_RR = (
    os.getenv("TIMEFRAME", "1h"),
    int(os.getenv("UNIVERSE_SIZE", "30")),
    float(os.getenv("MIN_RR", "3.0")),
)
MAX_OPEN_POSITIONS = int(os.getenv("MAX_OPEN_POSITIONS", 3))
LOOP_DELAY, TIMEZONE, REPORT_HOUR, REPORT_WEEKDAY = (
    int(os.getenv("LOOP_DELAY", "5")),
    os.getenv("TIMEZONE", "Europe/Lisbon"),
    int(os.getenv("REPORT_HOUR", "21")),
    int(os.getenv("REPORT_WEEKDAY", "6")),
)
...
def sync_positions_on_startup(ex: ccxt.Exchange):
    ...
    try:
        ...
        if ghost_symbols:
            message = (
                "⚠️ <b>Positions Fantômes Détectées !</b>\n"
...
def select_and_execute_best_pending_signal(ex: ccxt.Exchange):
    ...
    for symbol, pending in list(state.pending_signals.items()):
        ...
        if df is None or df.index[-1] <= pending['candle_timestamp']:
            continue
        ...
        new_tp_price = (
            last_indicators['bb80_up'] if is_long and pending['signal']['regime'] == 'Tendance' else
            last_indicators['bb80_lo'] if (not is_long) and pending['signal']['regime'] == 'Tendance' else
            last_indicators['bb20_up'] if is_long else
            last_indicators['bb20_lo']
        )

        new_rr = (
            (new_tp_price - new_entry_price) / (new_entry_price - sl_price)
            if is_long and (new_entry_price - sl_price) > 0
            else (new_entry_price - new_tp_price) / (sl_price - new_entry_price)
            if (not is_long) and (sl_price - new_entry_price) > 0
            else 0
        )
    ...
    if not validated_signals:
        print("   -> Aucun signal n'a passé la re-validation du R/R.")
        return
...
def process_message(message: Dict):
    ...
    if command == "/start":
        ...
        notifier.send_config_message(
            min_rr=MIN_RR,
            risk=trader.RISK_PER_TRADE_PERCENT,
        ...
        if len(parts) < 2:
            notifier.tg_send("Usage: <code>/setuniverse &lt;nombre&gt;</code>")
            return
        try:
            ...
            if new_size > 0:
                ...
                notifier.tg_send(
                    f"✅ Taille du scan mise à jour à <b>{new_size}</b> paires.\n<i>(Sera appliqué au prochain redémarrage)</i>"
                )
            else:
                notifier.tg_send("❌ Le nombre doit être > 0.")
        except ValueError:
            notifier.tg_send("❌ Valeur invalide.")

    elif command == "/setmaxpos":
        if len(parts) < 2:
            notifier.tg_send("Usage: <code>/setmaxpos &lt;nombre&gt;</code>")
            return
        try:
            new_max = int(parts[1])
            if new_max >= 0:
    ...
    while True:
        try:
            with _lock:
                is_paused = _paused
            if is_paused:
                print("   -> (Moteur de Trading en pause)")
            ...
            notifier.tg_send("⛔ Arrêt manuel du thread de trading.")
            break
        except Exception:
            print("\n--- ERREUR CRITIQUE DANS LE THREAD DE TRADING ---")
            error_details = traceback.format_exc()
            print(error_details)
            notifier.tg_send_error("Erreur critique (Moteur de Trading)", error_details)
            print("--------------------------------------------------")
            time.sleep(15)
...
def main():
    ...
    if not database.get_setting('PAPER_TRADING_MODE'):
        database.set_setting('PAPER_TRADING_MODE', os.getenv("PAPER_TRADING_MODE", "true").lower())

    sync_positions_on_startup(ex)

    notifier.send_start_banner(
        "TESTNET" if BITGET_TESTNET else "LIVE",
        "PAPIER" if database.get_setting('PAPER_TRADING_MODE') == 'true' else "RÉEL",
