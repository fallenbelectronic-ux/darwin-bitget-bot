...
def get_recent_signals_message(hours: int) -> str:
    ...
    for s in signals_in_period:
        ts = datetime.fromtimestamp(s['timestamp'], tz=timezone.utc).astimezone(pytz.timezone(TIMEZONE)).strftime('%H:%M'); side_icon = "📈" if s['signal']['side'] == 'buy' else "📉"
...
def detect_signal(symbol: str, df: pd.DataFrame) -> Optional[Dict[str, Any]]:
    ...
    last_candle = df_with_indicators.iloc[-1]; is_uptrend = last_candle['close'] > last_candle['bb80_mid']; is_downtrend = last_candle['close'] < last_candle['bb80_mid']; signal = None
    ...
    if not signal:
        if (last_candle['low'] <= last_candle['bb20_lo'] and last_candle['low'] <= last_candle['bb80_lo'] and
            last_candle['close'] > last_candle['bb20_lo'] and last_candle['close'] > last_candle['bb80_lo']):
        ...
        elif (last_candle['high'] >= last_candle['bb20_up'] and last_candle['high'] >= last_candle['bb80_up'] and
              last_candle['close'] < last_candle['bb20_up'] and last_candle['close'] < last_candle['bb80_up']):
    ...
    if signal: signal['bb20_mid'] = last_candle['bb20_mid']; return signal
    return None
...
def main():
    ...
    while True:
        ...
        try:
            cleanup_recent_signals(); poll_telegram_updates(); check_scheduled_reports()
            if _paused: time.sleep(LOOP_DELAY); continue
            ...
            print("\n--- ERREUR CRITIQUE DANS LA BOUCLE PRINCIPALE ---"); error_details = traceback.format_exc(); print(error_details)
            notifier.tg_send_error("Erreur critique (boucle)", error_details); print("--------------------------------------------------")
            time.sleep(15)
