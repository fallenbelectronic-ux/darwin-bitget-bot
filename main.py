...
def detect_signal(symbol: str, df: pd.DataFrame) -> Optional[Dict[str, Any]]:
    """Logique de détection qui analyse les 2 bougies précédentes pour un contact."""
...
def process_callback_query(callback_query: Dict):
    ...
    if data == 'pause': _paused = True; notifier.tg_send("⏸️ Scan mis en pause.")
        ...
        database.set_setting('PAPER_TRADING_MODE', 'false'); notifier.tg_send("🚨 <b>ATTENTION:</b> Bot en mode <b>RÉEL</b>."); notifier.send_mode_message(BITGET_TESTNET, False)
    elif data == 'switch_to_PAPER':
        database.set_setting('PAPER_TRADING_MODE', 'true'); notifier.tg_send("✅ Bot en mode <b>PAPIER</b>."); notifier.send_mode_message(BITGET_TESTNET, True)
